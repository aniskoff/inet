...
class SimpleObserver : public Ipv4NetworkConfigurator
{
    ...
    private:
        ...
        void prepareRoutingTableForDynamicHop(const std::string& hopNodePath, const std::string& destNodePath);
        ...
        void applyRoutingBimask(std::vector<bool> bitmask);
        ...
        void readDynHopsToNextHopsConf();
        ...
        // Hop is "dynamic" if there can be some traffic enginerring in it i.e. we can manage it's next (two) hops.
        std::unordered_map<std::string, pair_ss> dynamicHopToNextHopCandidates; 
        ...
        std::string dynHopsToNextHopsFp;
        std::string srcDestToFstLstHopsFp;
        char csvSep = ',';  

        const int nFlows = 6;

        const std::vector<std::vector<bool>> routingBitmasks = generateRoutingBitmasks(nFlows);

        ClockEvent* mObserverTimer = nullptr;
