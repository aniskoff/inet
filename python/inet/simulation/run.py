...
def run_simulation(working_directory, sim_time_limit, sim_time_limit_factor = None, mode = "debug", ui = "Cmdenv", ini_file = "omnetpp.ini", config = "General", run = 0, print_end = "\n", extra_args = [], **kwargs):
    ...
    sys.stdout.flush()
    return subprocess.run(["inet", "--" + mode, "-s", "-u", ui, "-c", config, "-r", str(run), "--sim-time-limit", sim_time_limit, *extra_args], cwd = "/home/levy/workspace/inet" + working_directory, capture_output = True)

# TODO merge into run_simulation?
def run_simulation2(simulation, sim_time_limit = None, get_extra_args = None, ini_file = None, config = None, run = None, **kwargs):
    ...
    return run_simulation(working_directory = simulation['wd'], sim_time_limit = sim_time_limit, ini_file = ini_file, config = config, run = run, extra_args = extra_args, **kwargs)

def run_simulations(simulations = None, run_simulation = run_simulation2, **kwargs):
    if simulations is None:
        simulations = get_simulations(**kwargs)
    ...
    return pool.map(partial, simulations)
